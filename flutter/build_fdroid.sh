...
#
# Script to build F-Droid release of emudesk
#
# Copyright (C) 2024, The emudesk Authors
...
#
# - versionName from https://github.com/emudesk/emudesk/releases/download/fdroid-version/emudesk-version.txt
# - versionCode from https://github.com/emudesk/emudesk/releases/download/fdroid-version/emudesk-version.txt
...
arm64-v8a)
	...
	emudesk_FEATURES='flutter,hwcodec'
	;;
armeabi-v7a)
...
prebuild)
	...
	# Flutter used to compile main emudesk library
	...
	# Install Flutter version for emudesk library build
...
build)
	...
	cargo ndk \
		...
		--locked \
		--release \
		--features "${emudesk_FEATURES}"

	mkdir -p "flutter/android/app/src/main/jniLibs/${ANDROID_ABI}"

	cp "target/${RUST_TARGET}/release/libemudesk.so" \
		"flutter/android/app/src/main/jniLibs/${ANDROID_ABI}/libemudesk.so"
