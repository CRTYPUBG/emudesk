...
#define emudesk_LIB_PATH "libemudesk.so"
typedef bool (*emudeskCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

bool flutter_emudesk_core_main() {
   void* libemudesk = dlopen(emudesk_LIB_PATH, RTLD_LAZY);
   if (!libemudesk) {
      fprintf(stderr,"Failed to load \"libemudesk.so\"\n");
      char* error;
   ...
   auto core_main = (emudeskCoreMain) dlsym(libemudesk,"emudesk_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"emudesk_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
