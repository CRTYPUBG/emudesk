...
const String kWaylandKeyboardIssueUrl =
    'https://github.com/emudesk/emudesk/issues/14586';
...
List<TTextMenu> toolbarControls(BuildContext context, String id, FFI ffi) {
  ...
  // https://github.com/emudesk/emudesk/pull/9731
...
bool showVirtualDisplayMenu(FFI ffi) {
  ...
  if (ffi.ffiModel.pi.isEmuDeskIdd || ffi.ffiModel.pi.isAmyuniIdd) {
    return true;
  }
  return false;
}
...
List<Widget> getVirtualDisplayMenuChildren(
  ...
  final privacyModeState = PrivacyModeState.find(id);
  if (pi.isEmuDeskIdd) {
    final virtualDisplays = ffi.ffiModel.pi.EmuDeskVirtualDisplays;
