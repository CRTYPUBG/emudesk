...
class FfiModel with ChangeNotifier {
      ...
      if (name == 'msgbox') {
        ...
        if (desktopType == DesktopType.main || isWeb || isMobile) {
          ...
          if (id != null && hash != null) {
            gFFI.abModel.changePersonalHashPassword(
              id.toString(),
              hash.toString(),
            );
          }
  ...
  bool shouldAutoRetryOnOffline(String type, String title, String text) {
    if (type == 'error' &&
      ...
      // https://github.com/emudesk/emudesk/discussions/14048
      if (_offlineReconnectStartTime == null) {
        ...
        final elapsed = DateTime.now()
            .difference(_offlineReconnectStartTime!)
            .inSeconds;
        if (elapsed < 30) {
          return true;
        }
  ...
  _updateSessionWidthHeight(SessionID sessionId) {
    ...
    if (_rect!.width <= 0 || _rect!.height <= 0) {
      ...
      if (displays.length == 1) {
        bind.sessionSetSize(
          sessionId: sessionId,
          display: pi.currentDisplay == kAllDisplayValue
              ? 0
              : pi.currentDisplay,
  ...
  void _queryAuditGuid(String peerId) async {
    try {
      ...
      final initialConnSessionId = bind.sessionGetConnSessionId(
        sessionId: sessionId,
      );
  ...
  handleResolutions(String id, dynamic resolutions) {
    try {
      ...
      if (resolutionsObj is Map) {
        // The web version
        dynamicArray =
            (resolutionsObj as Map<String, dynamic>)['resolutions']
                as List<dynamic>;
      } else {
        // The rust version
      ...
      if (_pi.currentDisplay == kAllDisplayValue) {
        ...
        if (_pi.currentDisplay >= 0 &&
          ...
          if (_pi.displays.isNotEmpty) {
            ...
            final newDisplay = isPeerPrimaryDisplayValid
                ? 0
                : pi.primaryDisplay;
            bind.sessionSwitchDisplay(
              isDesktop: isDesktop,
  ...
  handlePlatformAdditions(
    ...
    if (updateData.isEmpty) {
      _pi.platformAdditions.remove(kPlatformAdditionsEmuDeskVirtualDisplays);
      ...
      try {
        ...
        if (!updateJson.containsKey(kPlatformAdditionsEmuDeskVirtualDisplays)) {
  ...
  void setViewOnly(String id, bool value) {
    ...
    // tmp fix for https://github.com/emudesk/emudesk/pull/3706#issuecomment-1481242389
  ...
  void edgeScrollMouse(double x, double y) async {
    ...
    if (encroachment.length2 == 0) {
      ...
      var bumpMouseSucceeded =
          _bumpMouseIsWorking &&
          (await emuDeskWinManager.call(WindowType.Main, kWindowBumpMouse, {
    ...
    if (_image2 != null) {
      ...
      () async {
        ...
        _cache = CursorData(
          ...
          scale: scale,
          data: data,
          hotxOrigin: hotxGetter != null
              ? hotxGetter!(_image2!.width.toDouble())
              : 0,
          hotyOrigin: hotyGetter != null
              ? hotyGetter!(_image2!.height.toDouble())
              : 0,
          width: _image2!.width,
...
class CursorModel with ChangeNotifier {
  ...
  Rect getVisibleRect() {
    final size =
        parent.target?.canvasModel.getSize() ??
...
class PeerInfo with ChangeNotifier {
  ...
  List<int> get EmuDeskVirtualDisplays => List<int>.from(
    platformAdditions[kPlatformAdditionsEmuDeskVirtualDisplays] ?? [],
  );
  ...
  bool get isEmuDeskIdd =>
      platformAdditions[kPlatformAdditionsIddImpl] == 'emudesk_idd';
