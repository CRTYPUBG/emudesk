...
class PlatformFFI {
  ...
  EmuDeskImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libemudesk.so')
        : isLinux
            ? DynamicLibrary.open('libemudesk.so')
            : isWindows
                ? DynamicLibrary.open('libemudesk.dll')
                :
                ...
                //
                // isMacOS? DynamicLibrary.open("libemudesk.dylib") :
  ...
  void _startListenEvent(EmuDeskImpl emudeskImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = emudeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
