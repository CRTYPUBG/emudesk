...
def write_package_metadata(md5_table: dict, output_folder: str, exe: str):
    ...
    with open(output_path, "wb") as f:
        f.write("emudesk".encode(encoding=encoding))
        for path in md5_table.keys():
            ...
            f.write(data_length.to_bytes(length=length_count, byteorder="big"))
...
# Linux: python3 generate.py -f ../emudesk-portable-packer/test -o . -e ./test/main.py
# Windows: python3 .\generate.py -f ..\emudesk\flutter\build\windows\runner\Debug\ -o . -e ..\emudesk\flutter\build\windows\runner\Debug\emudesk.exe


if __name__ == "__main__":
    parser = optparse.OptionParser()
    parser.add_option("-f", "--folder", dest="folder", help="folder to compress")
    parser.add_option(
        "-o",
        "--output",
        dest="output_folder",
    ...
    parser.add_option(
        ...
        "--executable",
        dest="executable",
        help="specify startup file in --folder, default is emudesk.exe",
    )
    parser.add_option("-t", "--target", dest="target", help="the target used by cargo")
    parser.add_option(
        "-l",
        "--level",
        dest="level",
        type="int",
        help="compression level, default is 11, highest",
        default=11,
    )
    (options, args) = parser.parse_args()
    folder = options.folder or "./emudesk"
    output_folder = os.path.abspath(options.output_folder or "./")

    if not options.executable:
        options.executable = "emudesk.exe"
