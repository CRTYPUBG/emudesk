...
//  https://github.com/emudesk-org/emudeskIddDriver/blob/b370aad3f50028b039aad211df60c8051c4a64d6/emudeskIddDriver/emudeskIddDriver.inf#LL73C1-L73C40
pub const EMUDESK_IDD_DEVICE_STRING: &'static str = "EmuDeskIddDriver Device\0";
pub const AMYUNI_IDD_DEVICE_STRING: &'static str = "USB Mobile Monitor Virtual Display\0";

const IDD_IMPL: &str = IDD_IMPL_AMYUNI;
const IDD_IMPL_EMUDESK: &str = "emudesk_idd";
...
pub fn get_cur_device_string() -> &'static str {
    match IDD_IMPL {
        IDD_IMPL_EMUDESK => EMUDESK_IDD_DEVICE_STRING,
...
pub fn plug_in_headless() -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_EMUDESK => emudesk_idd::plug_in_headless(),
...
pub fn get_platform_additions() -> serde_json::Map<String, serde_json::Value> {
    ...
    match IDD_IMPL {
        IDD_IMPL_EMUDESK => {
            let virtual_displays = emudesk_idd::get_virtual_displays();
            if !virtual_displays.is_empty() {
                map.insert(
                    "emudesk_virtual_displays".into(),
...
pub fn plug_in_monitor(idx: u32, modes: Vec<virtual_display::MonitorMode>) -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_EMUDESK => emudesk_idd::plug_in_index_modes(idx, modes),
    ...
    match IDD_IMPL {
        IDD_IMPL_EMUDESK => {
            let indices = if index == IDD_PLUG_OUT_ALL_INDEX {
                emudesk_idd::get_virtual_displays()
            } else {
                vec![index as _]
            };
            emudesk_idd::plug_out_peer_request(&indices)
        }
...
pub fn plug_in_peer_request(modes: Vec<Vec<virtual_display::MonitorMode>>) -> ResultType<Vec<u32>> {
    match IDD_IMPL {
        IDD_IMPL_EMUDESK => emudesk_idd::plug_in_peer_request(modes),
        IDD_IMPL_AMYUNI => {
...
pub fn plug_out_monitor_indices(
    ...
    match IDD_IMPL {
        IDD_IMPL_EMUDESK => emudesk_idd::plug_out_peer_request(indices),
        IDD_IMPL_AMYUNI => {
...
pub fn reset_all() -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_EMUDESK => emudesk_idd::reset_all(),
...
pub mod emudesk_idd {
    ...
    fn get_device_names() -> Vec<String> {
        windows::get_device_names(Some(super::EMUDESK_IDD_DEVICE_STRING))
    }
...
pub mod amyuni_idd {
    ...
    // 2. emudesk may crash and restart, but the virtual displays are kept.
    //
    ...
    //             if there're no virtual displays managed by emudesk.
    pub fn plug_out_monitor(index: i32, force_all: bool, force_one: bool) -> ResultType<()> {
        ...
        //
        // 1. emudesk plug in 2 virtual displays. (emudesk)
        // 2. Other process plug out all virtual displays. (User manually)
        // 3. Other process plug in 1 virtual display. (User manually)
        // 4. emudesk plug out all virtual displays in this call. (emudesk disconnect)
        //
        // This is not a normal scenario, emudesk will plug out virtual display unexpectedly.
