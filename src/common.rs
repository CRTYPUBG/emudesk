...
pub const PORTABLE_APPNAME_RUNTIME_ENV_KEY: &str = "emudesk_APPNAME";
...
// Because the url is always `https://api.emudesk.com/version/latest`.
#[tokio::main(flavor = "current_thread")]
pub async fn do_check_software_update() -> hbb_common::ResultType<()> {
    let (request, url) =
        hbb_common::version_check_request(hbb_common::VER_TYPE_emudesk_CLIENT.to_string());
...
#[inline]
pub fn is_emudesk() -> bool {
    hbb_common::config::APP_NAME.read().unwrap().eq("EmuDesk")
}
...
fn get_api_server_(api: String, custom: String) -> String {
    ...
    "https://admin.emudesk.com".to_owned()
}

#[inline]
pub fn is_public(url: &str) -> bool {
    let url = url.to_ascii_lowercase();
    url.contains("emudesk.com/") || url.ends_with("emudesk.com")
}
...
pub fn check_process(arg: &str, mut same_uid: bool) -> bool {
    ...
    for (_, p) in sys.processes().iter() {
        ...
        // on mac, p.cmd() get "/Applications/emudesk.app/Contents/MacOS/emudesk", "XPC_SERVICE_NAME=com.carriez.emudesk_server"
...
pub type EmuDeskInterval = ThrottledInterval;

#[inline]
pub fn emudesk_interval(i: Interval) -> ThrottledInterval {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_emudesk_interval() {
        ...
        for maker in base_intervals.into_iter() {
            ...
            let mut timer = emudesk_interval(maker());
    ...
    #[allow(non_snake_case)]
    #[tokio::test]
    async fn test_emudesk_interval_sleep() {
        let base_intervals = [interval_maker, interval_at_maker];
    ...
    #[test]
    fn test_is_public() {
        // Test URLs containing "emudesk.com/"
        assert!(is_public("https://emudesk.com/"));
        assert!(is_public("https://www.emudesk.com/"));
        assert!(is_public("https://api.emudesk.com/v1"));
        assert!(is_public("https://API.emudesk.COM/v1"));
        assert!(is_public("https://emudesk.com/path"));

        // Test URLs ending with "emudesk.com"
        assert!(is_public("emudesk.com"));
        assert!(is_public("https://emudesk.com"));
        assert!(is_public("https://emudesk.com"));
        assert!(is_public("http://www.emudesk.com"));
        assert!(is_public("https://api.emudesk.com"));
        ...
        assert!(!is_public("https://emudesk.computer.com"));
        assert!(!is_public("emudesk.comhello.com"));
    }
    ...
    fn test_should_use_tcp_proxy_for_api_url() {
        ...
        assert!(!should_use_tcp_proxy_for_api_url(
            "https://admin.emudesk.com/api/login",
            "https://admin.emudesk.com"
        ));
        assert!(!should_use_tcp_proxy_for_api_url(
            "https://admin.example.com/api/login",
            "not a url"
