// https://github.com/emudesk/emudesk/discussions/6444#discussioncomment-9010062
...
//*************************************************************
//
//  DeleteemudeskTestCertsW_SingleHive()
//
//  Purpose:    Deletes emudesk Test certificates and wrong key stores
//
...
BOOL DeleteemudeskTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteemudeskTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteemudeskTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteemudeskTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteemudeskTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
