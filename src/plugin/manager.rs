...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    // vec![PluginSource {
    //     name: "emudesk".to_string(),
    //     url: "https://raw.githubusercontent.com/fufesou/emudesk-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "emudesk wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "emudesk wants to uninstall the plugin",
    )
}
