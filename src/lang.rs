...
pub fn translate_locale(name: String, locale: &str) -> String {
    ...
    let replace = |s: &&str| {
        ...
        if !crate::is_emudesk() {
            if s.contains("emudesk")
                && !name.starts_with("upgrade_emudesk_server_pro")
                && name != "powered_by_me"
            {
                let app_name = crate::get_app_name();
                if !app_name.contains("emudesk") {
                    s = s.replace("emudesk", &app_name);
                } else {
                    // https://github.com/emudesk/emudesk-server-pro/issues/845
                    // If app_name contains "emudesk" (e.g., "emudesk-Admin"), we need to avoid
                    // replacing "emudesk" within the already-substituted app_name, which would
                    // cause duplication like "emudesk-Admin" -> "emudesk-Admin-Admin".
                    //
